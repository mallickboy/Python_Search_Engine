from get_links import crawling as bot
        ...
        while i < len_starting_links:
            try:
                ...
                for j,thread in enumerate(started_threads):  # Loop over the started threads
                    ...
                    # all_links.update(self.store_all[j])  # Update all_links with the links 
                    all_links2.append(self.store_all[j])
                    i+=1
...
bc=batch_crawling()
surl=bc.get_starting_urls('./new_data/starting_urls/python_tutorial2.json','link')
# print(surl)
th=bc.get_n_threads(5)
print(th)
bc.handle_all_threads(surl,1,4)
print(len(all_links))